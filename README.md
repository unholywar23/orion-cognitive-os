...
<img src="apple-touch-icon.png" width="80" alt="Orion logo">

# Orion — the AI that remembers you

**A private cognitive OS for Windows. Its memory is files on YOUR disk —<br>not rows in someone else's cloud.**
...
<img src="app-recall.png" width="880" alt="A brand-new conversation: 'What did we settle on for Atlas pricing, and what's my rule about new features?' Orion's visible thought process reasons over its own memory, the answer recalls the exact pricing decision — and it honestly flags that two different versions of the feature rule exist in memory instead of papering over it. The right panel lists the 8 memories recalled for this reply; model and provider stay masked by default.">

<sub>Captured live through the real product (demo scenario: "Atlas" pricing). Watch what actually happens: the visible <b>Thought process</b> reasons over memory in plain language, the answer recalls the decision from a previous conversation — and instead of smoothing it over, Orion flags that its memory holds <i>two different versions</i> of the feature rule and asks which one is canonical. That's the honesty layer working in public. The right panel lists the exact memories used; model/provider stay masked by Orion's own privacy default. One private memory row and my other conversations are blurred — the only edit. Nothing is simulated.</sub>
...
<sub>© 2026 Orion · Windows today · Your memory is files on your disk.</sub>
